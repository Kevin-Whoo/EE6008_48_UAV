...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d67 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu62 = paddle.nn.Silu()
        self.conv2d68 = paddle.nn.Conv2D(padding=1, groups=64, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu63 = paddle.nn.Silu()
        self.conv2d69 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu64 = paddle.nn.Silu()
        self.conv2d70 = paddle.nn.Conv2D(out_channels=10, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d75 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        self.silu68 = paddle.nn.Silu()
        self.conv2d76 = paddle.nn.Conv2D(padding=1, groups=64, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu69 = paddle.nn.Silu()
        self.conv2d77 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu70 = paddle.nn.Silu()
        self.conv2d78 = paddle.nn.Conv2D(out_channels=10, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d83 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=256)
        self.silu74 = paddle.nn.Silu()
        self.conv2d84 = paddle.nn.Conv2D(padding=1, groups=64, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu75 = paddle.nn.Silu()
        self.conv2d85 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        self.silu76 = paddle.nn.Silu()
        self.conv2d86 = paddle.nn.Conv2D(out_channels=10, kernel_size=(1, 1), in_channels=64)
        ...
        self.x1156 = self.create_parameter(dtype='float32', shape=(1, 2, 8400), default_initializer=paddle.nn.initializer.Constant(value=0.0))
    ...
    def forward(self, x0):
        ...
        x871 = 2
        x875 = 1
        x901 = self.conv2d63(x677)
        x902 = self.silu59(x901)
        x910 = self.conv2d64(x902)
        x911 = self.silu60(x910)
        x918 = self.conv2d65(x911)
        x931 = self.conv2d66(x677)
        x932 = self.silu61(x931)
        x940 = self.conv2d67(x932)
        x941 = self.silu62(x940)
        x951 = self.conv2d68(x941)
        x952 = self.silu63(x951)
        x960 = self.conv2d69(x952)
        x961 = self.silu64(x960)
        x968 = self.conv2d70(x961)
        x969 = [x918, x968]
        x970 = paddle.concat(x=x969, axis=1)
        x981 = self.conv2d71(x744)
        x982 = self.silu65(x981)
        x990 = self.conv2d72(x982)
        x991 = self.silu66(x990)
        x998 = self.conv2d73(x991)
        x1011 = self.conv2d74(x744)
        x1012 = self.silu67(x1011)
        x1020 = self.conv2d75(x1012)
        x1021 = self.silu68(x1020)
        x1031 = self.conv2d76(x1021)
        x1032 = self.silu69(x1031)
        x1040 = self.conv2d77(x1032)
        x1041 = self.silu70(x1040)
        x1048 = self.conv2d78(x1041)
        x1049 = [x998, x1048]
        x1050 = paddle.concat(x=x1049, axis=1)
        x1061 = self.conv2d79(x867)
        x1062 = self.silu71(x1061)
        x1070 = self.conv2d80(x1062)
        x1071 = self.silu72(x1070)
        x1078 = self.conv2d81(x1071)
        x1091 = self.conv2d82(x867)
        x1092 = self.silu73(x1091)
        x1100 = self.conv2d83(x1092)
        x1101 = self.silu74(x1100)
        x1111 = self.conv2d84(x1101)
        x1112 = self.silu75(x1111)
        x1120 = self.conv2d85(x1112)
        x1121 = self.silu76(x1120)
        x1128 = self.conv2d86(x1121)
        x1129 = [x1078, x1128]
        x1130 = paddle.concat(x=x1129, axis=1)
        x1132 = paddle.reshape(x=x970, shape=[1, 74, -1])
        x1134 = paddle.reshape(x=x1050, shape=[1, 74, -1])
        x1136 = paddle.reshape(x=x1130, shape=[1, 74, -1])
        x1137 = [x1132, x1134, x1136]
        x1138 = paddle.concat(x=x1137, axis=2)
        x1140 = paddle.split(x=x1138, num_or_sections=[64, 10], axis=1)
        x1141, x1142 = x1140
        x1145 = paddle.reshape(x=x1141, shape=[1, 4, 16, 8400])
        x1146_shape = x1145.shape
        x1146_len = len(x1146_shape)
        x1146_list = []
        for i in range(x1146_len):
            x1146_list.append(i)
        if x871 < 0:
            x871_new = x871 + x1146_len
        else:
            x871_new = x871
        if x875 < 0:
            x875_new = x875 + x1146_len
        else:
            x875_new = x875
        x1146_list[x871_new] = x875_new
        x1146_list[x875_new] = x871_new
        x1146 = paddle.transpose(x=x1145, perm=x1146_list)
        x1147 = self.softmax1(x1146)
        x1153 = self.conv2d87(x1147)
        x1155 = paddle.reshape(x=x1153, shape=[1, 4, 8400])
        x1156 = self.x1156
        x1157 = paddle.split(x=x1155, num_or_sections=2, axis=1)
        x1158, x1159 = x1157
        x1160 = x1156 - x1158
        x1161 = x1156 + x1159
        x1162 = x1160 + x1161
        ...
        x1164 = x1161 - x1160
        x1165 = [x1163, x1164]
        x1166 = paddle.concat(x=x1165, axis=1)
        x1167 = x1166 * x868
        x1168 = self.sigmoid0(x1142)
        x1169 = [x1167, x1168]
        x1170 = paddle.concat(x=x1169, axis=1)
        return x1170
...
def main(x0):
    ...
    params = paddle.load(r'/scratch/users/ntu/khu005/project/yolo11n/best_paddle_model/model.pdparams')
